...
// CHECK-LABEL: module attributes {llzk.main = !struct.type<@InnerConditional6<[4]>>, veridise.lang = "llzk"} {
// CHECK-NEXT:    struct.def @InnerConditional6<[@N]> {
// CHECK-NEXT:      struct.member @out : !array.type<@N x !felt.type> {llzk.pub}
// CHECK-NEXT:      function.def @compute(%[[VAL_0:[0-9a-zA-Z_\.]+]]: !array.type<@N x !felt.type>) -> !struct.type<@InnerConditional6<[@N]>> attributes {function.allow_non_native_field_ops, function.allow_witness} {
// CHECK-NEXT:        %[[VAL_1:[0-9a-zA-Z_\.]+]] = struct.new : <@InnerConditional6<[@N]>>
...
// CHECK-NEXT:        }
// CHECK-NEXT:        struct.writem %[[VAL_1]][@out] = %[[VAL_3]] : <@InnerConditional6<[@N]>>, !array.type<@N x !felt.type>
// CHECK-NEXT:        function.return %[[VAL_1]] : !struct.type<@InnerConditional6<[@N]>>
// CHECK-NEXT:      }
...
// CHECK-NEXT:        %{{[0-9a-zA-Z_\.]+}} = struct.readm %[[VAL_20]][@out] : <@InnerConditional6<[@N]>>, !array.type<@N x !felt.type>
...
// CHECK-NEXT:          scf.condition(%[[VAL_26]]) %[[VAL_25]] : !felt.type
// CHECK-NEXT:        } do {
// CHECK-NEXT:        ^bb0(%[[VAL_27:[0-9a-zA-Z_\.]+]]: !felt.type):
// CHECK-NEXT:          %[[VAL_28:[0-9a-zA-Z_\.]+]] = felt.const  0
// CHECK-NEXT:          %[[VAL_29:[0-9a-zA-Z_\.]+]] = cast.toindex %[[VAL_27]]
// CHECK-NEXT:          %[[VAL_30:[0-9a-zA-Z_\.]+]] = array.read %[[VAL_21]]{{\[}}%[[VAL_29]]] : <@N x !felt.type>, !felt.type
// CHECK-NEXT:          %[[VAL_31:[0-9a-zA-Z_\.]+]] = felt.const  0
// CHECK-NEXT:          %[[VAL_32:[0-9a-zA-Z_\.]+]] = bool.cmp eq(%[[VAL_30]], %[[VAL_31]])
// CHECK-NEXT:          %[[VAL_33:[0-9a-zA-Z_\.]+]] = scf.if %[[VAL_32]] -> (!felt.type) {
// CHECK-NEXT:            %[[VAL_34:[0-9a-zA-Z_\.]+]] = felt.const  999
// CHECK-NEXT:            scf.yield %[[VAL_34]] : !felt.type
// CHECK-NEXT:          } else {
// CHECK-NEXT:            %[[VAL_35:[0-9a-zA-Z_\.]+]] = felt.const  888
// CHECK-NEXT:            scf.yield %[[VAL_35]] : !felt.type
// CHECK-NEXT:          }
// CHECK-NEXT:          %[[VAL_36:[0-9a-zA-Z_\.]+]] = felt.const  1
// CHECK-NEXT:          %[[VAL_37:[0-9a-zA-Z_\.]+]] = felt.add %[[VAL_27]], %[[VAL_36]] : !felt.type, !felt.type
