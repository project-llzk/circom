...
// CHECK-SAME:      (%[[VAL_0:.*]]: !felt.type, %[[VAL_1:.*]]: !felt.type) -> !struct.type<@CallDiffTypeTest<[]>> attributes {function.allow_witness} {
// CHECK-NEXT:        %[[VAL_2:.*]] = struct.new : <@CallDiffTypeTest<[]>>
// CHECK-NEXT:        %[[VAL_3:.*]] = function.call @f(%[[VAL_0]], %[[VAL_1]]) : (!felt.type, !felt.type) -> !felt.type
// CHECK-NEXT:        struct.writef %[[VAL_2]][@out1] = %[[VAL_3]] : <@CallDiffTypeTest<[]>>, !felt.type
// CHECK-NEXT:        %[[VAL_4:.*]] = felt.const  1
// CHECK-NEXT:        %[[VAL_5:.*]] = function.call @f(%[[VAL_4]], %[[VAL_1]]) : (!felt.type, !felt.type) -> !felt.type
...
// CHECK-SAME:      (%[[VAL_6:.*]]: !struct.type<@CallDiffTypeTest<[]>>, %[[VAL_7:.*]]: !felt.type, %[[VAL_8:.*]]: !felt.type) attributes {function.allow_constraint} {
// CHECK-NEXT:        %[[VAL_9:.*]] = function.call @f(%[[VAL_7]], %[[VAL_8]]) : (!felt.type, !felt.type) -> !felt.type
// CHECK-NEXT:        %[[VAL_10:.*]] = struct.readf %[[VAL_6]][@out1] : <@CallDiffTypeTest<[]>>, !felt.type
// CHECK-NEXT:        constrain.eq %[[VAL_10]], %[[VAL_9]] : !felt.type, !felt.type
// CHECK-NEXT:        %[[VAL_11:.*]] = felt.const  1
// CHECK-NEXT:        %[[VAL_12:.*]] = function.call @f(%[[VAL_11]], %[[VAL_8]]) : (!felt.type, !felt.type) -> !felt.type
// CHECK-NEXT:        %[[VAL_13:.*]] = struct.readf %[[VAL_6]][@out2] : <@CallDiffTypeTest<[]>>, !felt.type
