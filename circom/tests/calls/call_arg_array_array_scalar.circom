...
// CHECK-NEXT:      function.def @sum(%[[VAL_0:[0-9a-zA-Z_\.]+]]: !poly.tvar<@T_arg0> {function.arg_name = "a"}, %[[VAL_1:[0-9a-zA-Z_\.]+]]: !poly.tvar<@T_arg1> {function.arg_name = "b"}, %[[VAL_2:[0-9a-zA-Z_\.]+]]: !poly.tvar<@T_arg2> {function.arg_name = "c"}) -> !poly.tvar<@T_return> attributes {function.allow_non_native_field_ops} {
...
// CHECK-NEXT:        }
// CHECK-NEXT:        function.def @constrain(%[[VAL_66:[0-9a-zA-Z_\.]+]]: !struct.type<@CallArgTest::@CallArgTest<[]>>, %[[VAL_67:[0-9a-zA-Z_\.]+]]: !array.type<4,2,3 x !felt.type<"bn128">> {function.arg_name = "x"}, %[[VAL_68:[0-9a-zA-Z_\.]+]]: !array.type<2,3 x !felt.type<"bn128">> {function.arg_name = "y"}, %[[VAL_69:[0-9a-zA-Z_\.]+]]: !felt.type<"bn128"> {function.arg_name = "z"}) attributes {function.allow_constraint, function.allow_non_native_field_ops} {
