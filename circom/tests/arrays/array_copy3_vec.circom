...
// CHECK-NEXT:        }
// CHECK-NEXT:        function.def @constrain(%[[VAL_4:[0-9a-zA-Z_\.]+]]: !struct.type<@Array3::@Array3<[@n]>>, %[[VAL_5:[0-9a-zA-Z_\.]+]]: !array.type<@n,@n x !felt.type<"bn128">> {function.arg_name = "inp"}) attributes {function.allow_constraint, function.allow_non_native_field_ops} {
// CHECK-NEXT:          %[[VAL_6:[0-9a-zA-Z_\.]+]] = poly.read_const @n : index
// CHECK-NEXT:          %[[VAL_7:[0-9a-zA-Z_\.]+]] = cast.tofelt %[[VAL_6]] : index, !felt.type<"bn128">
// CHECK-NEXT:          %[[VAL_8:[0-9a-zA-Z_\.]+]] = struct.readm %[[VAL_4]][@out] : <@Array3::@Array3<[@n]>>, !array.type<@n,@n x !felt.type<"bn128">>
// CHECK-NEXT:          constrain.eq %[[VAL_8]], %[[VAL_5]] : !array.type<@n,@n x !felt.type<"bn128">>, !array.type<@n,@n x !felt.type<"bn128">>
