...
// CHECK-NEXT:        function.def @compute(%[[VAL_56:[0-9a-zA-Z_\.]+]]: !felt.type) -> !struct.type<@parity::@parity<[@n]>> attributes {function.allow_non_native_field_ops, function.allow_witness} {
...
// CHECK-NEXT:          %[[VAL_70:[0-9a-zA-Z_\.]+]] = struct.readm %[[VAL_69]][@out] : <@Num2Bits::@Num2Bits<[@n]>>, !array.type<@n x !felt.type>
// CHECK-NEXT:          %[[VAL_71:[0-9a-zA-Z_\.]+]] = felt.const  0
// CHECK-NEXT:          %[[VAL_72:[0-9a-zA-Z_\.]+]] = cast.toindex %[[VAL_71]] : !felt.type
// CHECK-NEXT:          %[[VAL_73:[0-9a-zA-Z_\.]+]] = array.read %[[VAL_70]]{{\[}}%[[VAL_72]]] : <@n x !felt.type>, !felt.type
// CHECK-NEXT:          struct.writem %[[VAL_57]][@out] = %[[VAL_73]] : <@parity::@parity<[@n]>>, !felt.type
// CHECK-NEXT:          %[[VAL_74:[0-9a-zA-Z_\.]+]] = pod.read %[[VAL_60]][@comp] : <[@count: index, @comp: !struct.type<@Num2Bits::@Num2Bits<[@n]>>, @params: !pod.type<[]>]>, !struct.type<@Num2Bits::@Num2Bits<[@n]>>
// CHECK-NEXT:          %[[VAL_75:[0-9a-zA-Z_\.]+]] = struct.readm %[[VAL_74]][@out] : <@Num2Bits::@Num2Bits<[@n]>>, !array.type<@n x !felt.type>
// CHECK-NEXT:          struct.writem %[[VAL_57]][@check$inputs] = %[[VAL_61]] : <@parity::@parity<[@n]>>, !pod.type<[@in: !felt.type]>
// CHECK-NEXT:          %[[VAL_76:[0-9a-zA-Z_\.]+]] = pod.read %[[VAL_60]][@comp] : <[@count: index, @comp: !struct.type<@Num2Bits::@Num2Bits<[@n]>>, @params: !pod.type<[]>]>, !struct.type<@Num2Bits::@Num2Bits<[@n]>>
// CHECK-NEXT:          struct.writem %[[VAL_57]][@check] = %[[VAL_76]] : <@parity::@parity<[@n]>>, !struct.type<@Num2Bits::@Num2Bits<[@n]>>
// CHECK-NEXT:          function.return %[[VAL_57]] : !struct.type<@parity::@parity<[@n]>>
// CHECK-NEXT:        }
// CHECK-NEXT:        function.def @constrain(%[[VAL_77:[0-9a-zA-Z_\.]+]]: !struct.type<@parity::@parity<[@n]>>, %[[VAL_78:[0-9a-zA-Z_\.]+]]: !felt.type) attributes {function.allow_constraint, function.allow_non_native_field_ops} {
// CHECK-NEXT:          %[[VAL_79:[0-9a-zA-Z_\.]+]] = poly.read_const @n : !felt.type
// CHECK-NEXT:          %[[VAL_80:[0-9a-zA-Z_\.]+]] = struct.readm %[[VAL_77]][@out] : <@parity::@parity<[@n]>>, !felt.type
// CHECK-NEXT:          %[[VAL_81:[0-9a-zA-Z_\.]+]] = struct.readm %[[VAL_77]][@check] : <@parity::@parity<[@n]>>, !struct.type<@Num2Bits::@Num2Bits<[@n]>>
// CHECK-NEXT:          %[[VAL_82:[0-9a-zA-Z_\.]+]] = struct.readm %[[VAL_77]][@check$inputs] : <@parity::@parity<[@n]>>, !pod.type<[@in: !felt.type]>
// CHECK-NEXT:          %[[VAL_83:[0-9a-zA-Z_\.]+]] = pod.read %[[VAL_82]][@in] : <[@in: !felt.type]>, !felt.type
// CHECK-NEXT:          constrain.eq %[[VAL_83]], %[[VAL_78]] : !felt.type, !felt.type
// CHECK-NEXT:          %[[VAL_84:[0-9a-zA-Z_\.]+]] = struct.readm %[[VAL_81]][@out] : <@Num2Bits::@Num2Bits<[@n]>>, !array.type<@n x !felt.type>
// CHECK-NEXT:          %[[VAL_85:[0-9a-zA-Z_\.]+]] = felt.const  0
// CHECK-NEXT:          %[[VAL_86:[0-9a-zA-Z_\.]+]] = cast.toindex %[[VAL_85]] : !felt.type
// CHECK-NEXT:          %[[VAL_87:[0-9a-zA-Z_\.]+]] = array.read %[[VAL_84]]{{\[}}%[[VAL_86]]] : <@n x !felt.type>, !felt.type
// CHECK-NEXT:          constrain.eq %[[VAL_80]], %[[VAL_87]] : !felt.type, !felt.type
// CHECK-NEXT:          %[[VAL_88:[0-9a-zA-Z_\.]+]] = struct.readm %[[VAL_81]][@out] : <@Num2Bits::@Num2Bits<[@n]>>, !array.type<@n x !felt.type>
// CHECK-NEXT:          %[[VAL_89:[0-9a-zA-Z_\.]+]] = pod.read %[[VAL_82]][@in] : <[@in: !felt.type]>, !felt.type
// CHECK-NEXT:          function.call @Num2Bits::@Num2Bits::@constrain(%[[VAL_81]], %[[VAL_89]]) : (!struct.type<@Num2Bits::@Num2Bits<[@n]>>, !felt.type) -> ()
// CHECK-NEXT:          function.return
// CHECK-NEXT:        }
// CHECK-NEXT:      }
// CHECK-NEXT:    }
