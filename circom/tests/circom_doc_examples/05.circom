...
// CHECK-NEXT:          %[[VAL_4:[0-9a-zA-Z_\.]+]] = function.call @Example::@Example::@compute() : () -> !struct.type<@Example::@Example<[]>>
// CHECK-NEXT:          %[[VAL_5:[0-9a-zA-Z_\.]+]] = pod.new { @comp = %[[VAL_4]] }  : <[@count: index, @comp: !struct.type<@Example::@Example<[]>>, @params: !pod.type<[]>]>
// CHECK-NEXT:          %[[VAL_6:[0-9a-zA-Z_\.]+]] = pod.new : <[]>
// CHECK-NEXT:          struct.writem %[[VAL_2]][@example$inputs] = %[[VAL_6]] : <@UsingExample::@UsingExample<[]>>, !pod.type<[]>
// CHECK-NEXT:          %[[VAL_7:[0-9a-zA-Z_\.]+]] = pod.read %[[VAL_5]][@comp] : <[@count: index, @comp: !struct.type<@Example::@Example<[]>>, @params: !pod.type<[]>]>, !struct.type<@Example::@Example<[]>>
// CHECK-NEXT:          struct.writem %[[VAL_2]][@example] = %[[VAL_7]] : <@UsingExample::@UsingExample<[]>>, !struct.type<@Example::@Example<[]>>
// CHECK-NEXT:          function.return %[[VAL_2]] : !struct.type<@UsingExample::@UsingExample<[]>>
// CHECK-NEXT:        }
// CHECK-NEXT:        function.def @constrain(%[[VAL_8:[0-9a-zA-Z_\.]+]]: !struct.type<@UsingExample::@UsingExample<[]>>) attributes {function.allow_constraint, function.allow_non_native_field_ops} {
// CHECK-NEXT:          %[[VAL_9:[0-9a-zA-Z_\.]+]] = struct.readm %[[VAL_8]][@example] : <@UsingExample::@UsingExample<[]>>, !struct.type<@Example::@Example<[]>>
// CHECK-NEXT:          %[[VAL_10:[0-9a-zA-Z_\.]+]] = struct.readm %[[VAL_8]][@example$inputs] : <@UsingExample::@UsingExample<[]>>, !pod.type<[]>
// CHECK-NEXT:          function.call @Example::@Example::@constrain(%[[VAL_9]]) : (!struct.type<@Example::@Example<[]>>) -> ()
// CHECK-NEXT:          function.return
// CHECK-NEXT:        }
// CHECK-NEXT:      }
// CHECK-NEXT:    }
