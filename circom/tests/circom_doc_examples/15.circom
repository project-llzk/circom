...
// CHECK-NEXT:      }
// CHECK-NEXT:      function.def @constrain(%[[VAL_9:[0-9a-zA-Z_\.]+]]: !struct.type<@mult<[]>>, %[[VAL_10:[0-9a-zA-Z_\.]+]]: !array.type<2 x !felt.type>) attributes {function.allow_constraint, function.allow_non_native_field_ops} {
...
// CHECK-NEXT:      struct.field @comp1 : !struct.type<@mult<[]>>
// CHECK-NEXT:      struct.field @comp1$inputs : !pod.type<[@in: !array.type<2 x !felt.type>]>
// CHECK-NEXT:      struct.field @comp2 : !struct.type<@mult<[]>>
// CHECK-NEXT:      struct.field @comp2$inputs : !pod.type<[@in: !array.type<2 x !felt.type>]>
// CHECK-NEXT:      function.def @compute(%[[VAL_19:[0-9a-zA-Z_\.]+]]: !array.type<4 x !felt.type>) -> !struct.type<@mult4<[]>> attributes {function.allow_non_native_field_ops, function.allow_witness} {
// CHECK-NEXT:        %[[VAL_20:[0-9a-zA-Z_\.]+]] = struct.new : <@mult4<[]>>
// CHECK-NEXT:        %[[VAL_21:[0-9a-zA-Z_\.]+]] = arith.constant 2 : index
// CHECK-NEXT:        %[[VAL_22:[0-9a-zA-Z_\.]+]] = pod.new { @count = %[[VAL_21]] }  : <[@count: index, @comp: !struct.type<@mult<[]>>, @params: !pod.type<[]>]>
// CHECK-NEXT:        %[[VAL_23:[0-9a-zA-Z_\.]+]] = pod.new : <[@in: !array.type<2 x !felt.type>]>
// CHECK-NEXT:        %[[VAL_24:[0-9a-zA-Z_\.]+]] = arith.constant 2 : index
// CHECK-NEXT:        %[[VAL_25:[0-9a-zA-Z_\.]+]] = pod.new { @count = %[[VAL_24]] }  : <[@count: index, @comp: !struct.type<@mult<[]>>, @params: !pod.type<[]>]>
...
// CHECK-NEXT:        %[[VAL_33:[0-9a-zA-Z_\.]+]] = pod.read %[[VAL_22]][@count] : <[@count: index, @comp: !struct.type<@mult<[]>>, @params: !pod.type<[]>]>, index
// CHECK-NEXT:        %[[VAL_34:[0-9a-zA-Z_\.]+]] = arith.constant 1 : index
// CHECK-NEXT:        %[[VAL_35:[0-9a-zA-Z_\.]+]] = arith.subi %[[VAL_33]], %[[VAL_34]] : index
// CHECK-NEXT:        pod.write %[[VAL_22]][@count] = %[[VAL_35]] : <[@count: index, @comp: !struct.type<@mult<[]>>, @params: !pod.type<[]>]>, index
...
// CHECK-NEXT:          %[[VAL_39:[0-9a-zA-Z_\.]+]] = function.call @mult::@compute(%[[VAL_38]]) : (!array.type<2 x !felt.type>) -> !struct.type<@mult<[]>>
// CHECK-NEXT:          pod.write %[[VAL_22]][@comp] = %[[VAL_39]] : <[@count: index, @comp: !struct.type<@mult<[]>>, @params: !pod.type<[]>]>, !struct.type<@mult<[]>>
// CHECK-NEXT:        } else {
// CHECK-NEXT:        }
...
// CHECK-NEXT:        %[[VAL_46:[0-9a-zA-Z_\.]+]] = pod.read %[[VAL_25]][@count] : <[@count: index, @comp: !struct.type<@mult<[]>>, @params: !pod.type<[]>]>, index
// CHECK-NEXT:        %[[VAL_47:[0-9a-zA-Z_\.]+]] = arith.constant 1 : index
// CHECK-NEXT:        %[[VAL_48:[0-9a-zA-Z_\.]+]] = arith.subi %[[VAL_46]], %[[VAL_47]] : index
// CHECK-NEXT:        pod.write %[[VAL_25]][@count] = %[[VAL_48]] : <[@count: index, @comp: !struct.type<@mult<[]>>, @params: !pod.type<[]>]>, index
...
// CHECK-NEXT:          %[[VAL_52:[0-9a-zA-Z_\.]+]] = function.call @mult::@compute(%[[VAL_51]]) : (!array.type<2 x !felt.type>) -> !struct.type<@mult<[]>>
// CHECK-NEXT:          pod.write %[[VAL_25]][@comp] = %[[VAL_52]] : <[@count: index, @comp: !struct.type<@mult<[]>>, @params: !pod.type<[]>]>, !struct.type<@mult<[]>>
// CHECK-NEXT:        } else {
// CHECK-NEXT:        }
...
// CHECK-NEXT:        %[[VAL_59:[0-9a-zA-Z_\.]+]] = pod.read %[[VAL_25]][@count] : <[@count: index, @comp: !struct.type<@mult<[]>>, @params: !pod.type<[]>]>, index
// CHECK-NEXT:        %[[VAL_60:[0-9a-zA-Z_\.]+]] = arith.constant 1 : index
// CHECK-NEXT:        %[[VAL_61:[0-9a-zA-Z_\.]+]] = arith.subi %[[VAL_59]], %[[VAL_60]] : index
// CHECK-NEXT:        pod.write %[[VAL_25]][@count] = %[[VAL_61]] : <[@count: index, @comp: !struct.type<@mult<[]>>, @params: !pod.type<[]>]>, index
...
// CHECK-NEXT:          %[[VAL_65:[0-9a-zA-Z_\.]+]] = function.call @mult::@compute(%[[VAL_64]]) : (!array.type<2 x !felt.type>) -> !struct.type<@mult<[]>>
// CHECK-NEXT:          pod.write %[[VAL_25]][@comp] = %[[VAL_65]] : <[@count: index, @comp: !struct.type<@mult<[]>>, @params: !pod.type<[]>]>, !struct.type<@mult<[]>>
// CHECK-NEXT:        } else {
// CHECK-NEXT:        }
...
// CHECK-NEXT:        %[[VAL_72:[0-9a-zA-Z_\.]+]] = pod.read %[[VAL_22]][@count] : <[@count: index, @comp: !struct.type<@mult<[]>>, @params: !pod.type<[]>]>, index
// CHECK-NEXT:        %[[VAL_73:[0-9a-zA-Z_\.]+]] = arith.constant 1 : index
// CHECK-NEXT:        %[[VAL_74:[0-9a-zA-Z_\.]+]] = arith.subi %[[VAL_72]], %[[VAL_73]] : index
// CHECK-NEXT:        pod.write %[[VAL_22]][@count] = %[[VAL_74]] : <[@count: index, @comp: !struct.type<@mult<[]>>, @params: !pod.type<[]>]>, index
...
// CHECK-NEXT:          %[[VAL_78:[0-9a-zA-Z_\.]+]] = function.call @mult::@compute(%[[VAL_77]]) : (!array.type<2 x !felt.type>) -> !struct.type<@mult<[]>>
// CHECK-NEXT:          pod.write %[[VAL_22]][@comp] = %[[VAL_78]] : <[@count: index, @comp: !struct.type<@mult<[]>>, @params: !pod.type<[]>]>, !struct.type<@mult<[]>>
// CHECK-NEXT:        } else {
// CHECK-NEXT:        }
// CHECK-NEXT:        struct.writef %[[VAL_20]][@comp1$inputs] = %[[VAL_23]] : <@mult4<[]>>, !pod.type<[@in: !array.type<2 x !felt.type>]>
// CHECK-NEXT:        %[[VAL_79:[0-9a-zA-Z_\.]+]] = pod.read %[[VAL_22]][@comp] : <[@count: index, @comp: !struct.type<@mult<[]>>, @params: !pod.type<[]>]>, !struct.type<@mult<[]>>
// CHECK-NEXT:        struct.writef %[[VAL_20]][@comp1] = %[[VAL_79]] : <@mult4<[]>>, !struct.type<@mult<[]>>
// CHECK-NEXT:        struct.writef %[[VAL_20]][@comp2$inputs] = %[[VAL_26]] : <@mult4<[]>>, !pod.type<[@in: !array.type<2 x !felt.type>]>
// CHECK-NEXT:        %[[VAL_80:[0-9a-zA-Z_\.]+]] = pod.read %[[VAL_25]][@comp] : <[@count: index, @comp: !struct.type<@mult<[]>>, @params: !pod.type<[]>]>, !struct.type<@mult<[]>>
// CHECK-NEXT:        struct.writef %[[VAL_20]][@comp2] = %[[VAL_80]] : <@mult4<[]>>, !struct.type<@mult<[]>>
// CHECK-NEXT:        function.return %[[VAL_20]] : !struct.type<@mult4<[]>>
// CHECK-NEXT:      }
// CHECK-NEXT:      function.def @constrain(%[[VAL_81:[0-9a-zA-Z_\.]+]]: !struct.type<@mult4<[]>>, %[[VAL_82:[0-9a-zA-Z_\.]+]]: !array.type<4 x !felt.type>) attributes {function.allow_constraint, function.allow_non_native_field_ops} {
// CHECK-NEXT:        %[[VAL_83:[0-9a-zA-Z_\.]+]] = struct.readf %[[VAL_81]][@comp1] : <@mult4<[]>>, !struct.type<@mult<[]>>
// CHECK-NEXT:        %[[VAL_84:[0-9a-zA-Z_\.]+]] = struct.readf %[[VAL_81]][@comp1$inputs] : <@mult4<[]>>, !pod.type<[@in: !array.type<2 x !felt.type>]>
// CHECK-NEXT:        %[[VAL_85:[0-9a-zA-Z_\.]+]] = struct.readf %[[VAL_81]][@comp2] : <@mult4<[]>>, !struct.type<@mult<[]>>
// CHECK-NEXT:        %[[VAL_86:[0-9a-zA-Z_\.]+]] = struct.readf %[[VAL_81]][@comp2$inputs] : <@mult4<[]>>, !pod.type<[@in: !array.type<2 x !felt.type>]>
...
// CHECK-NEXT:        %[[VAL_99:[0-9a-zA-Z_\.]+]] = cast.toindex %[[VAL_98]]
...
// CHECK-NEXT:        %[[VAL_102:[0-9a-zA-Z_\.]+]] = cast.toindex %[[VAL_101]]
...
// CHECK-NEXT:        %[[VAL_106:[0-9a-zA-Z_\.]+]] = cast.toindex %[[VAL_105]]
...
// CHECK-NEXT:        %[[VAL_109:[0-9a-zA-Z_\.]+]] = cast.toindex %[[VAL_108]]
...
// CHECK-NEXT:        function.call @mult::@constrain(%[[VAL_83]], %[[VAL_115]]) : (!struct.type<@mult<[]>>, !array.type<2 x !felt.type>) -> ()
// CHECK-NEXT:        %[[VAL_116:[0-9a-zA-Z_\.]+]] = pod.read %[[VAL_86]][@in] : <[@in: !array.type<2 x !felt.type>]>, !array.type<2 x !felt.type>
// CHECK-NEXT:        function.call @mult::@constrain(%[[VAL_85]], %[[VAL_116]]) : (!struct.type<@mult<[]>>, !array.type<2 x !felt.type>) -> ()
// CHECK-NEXT:        function.return
// CHECK-NEXT:      }
// CHECK-NEXT:    }
// CHECK-NEXT:  }
