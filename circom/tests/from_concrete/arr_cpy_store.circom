...
// CHECK-NEXT:      struct.field @c : !struct.type<@Sum<[]>>
// CHECK-NEXT:      struct.field @c$inputs : !pod.type<[@inp: !array.type<2 x !felt.type>]>
// CHECK-NEXT:      function.def @compute(%[[VAL_0:[0-9a-zA-Z_\.]+]]: !array.type<2 x !felt.type>, %[[VAL_1:[0-9a-zA-Z_\.]+]]: !array.type<2 x !felt.type>) -> !struct.type<@Foo<[]>> attributes {function.allow_non_native_field_ops, function.allow_witness} {
// CHECK-NEXT:        %[[VAL_2:[0-9a-zA-Z_\.]+]] = struct.new : <@Foo<[]>>
// CHECK-NEXT:        %[[VAL_3:[0-9a-zA-Z_\.]+]] = arith.constant 2 : index
// CHECK-NEXT:        %[[VAL_4:[0-9a-zA-Z_\.]+]] = pod.new { @count = %[[VAL_3]] }  : <[@count: index, @comp: !struct.type<@Sum<[]>>, @params: !pod.type<[]>]>
...
// CHECK-NEXT:          %[[VAL_12:[0-9a-zA-Z_\.]+]] = bool.cmp le(%[[VAL_10]], %[[VAL_11]])
...
// CHECK-NEXT:          %[[VAL_15:[0-9a-zA-Z_\.]+]] = pod.read %[[VAL_4]][@count] : <[@count: index, @comp: !struct.type<@Sum<[]>>, @params: !pod.type<[]>]>, index
// CHECK-NEXT:          %[[VAL_16:[0-9a-zA-Z_\.]+]] = arith.constant 1 : index
// CHECK-NEXT:          %[[VAL_17:[0-9a-zA-Z_\.]+]] = arith.subi %[[VAL_15]], %[[VAL_16]] : index
// CHECK-NEXT:          pod.write %[[VAL_4]][@count] = %[[VAL_17]] : <[@count: index, @comp: !struct.type<@Sum<[]>>, @params: !pod.type<[]>]>, index
...
// CHECK-NEXT:            %[[VAL_21:[0-9a-zA-Z_\.]+]] = function.call @Sum::@compute(%[[VAL_20]]) : (!array.type<2 x !felt.type>) -> !struct.type<@Sum<[]>>
// CHECK-NEXT:            pod.write %[[VAL_4]][@comp] = %[[VAL_21]] : <[@count: index, @comp: !struct.type<@Sum<[]>>, @params: !pod.type<[]>]>, !struct.type<@Sum<[]>>
// CHECK-NEXT:          } else {
// CHECK-NEXT:          }
...
// CHECK-NEXT:        }
// CHECK-NEXT:        struct.writef %[[VAL_2]][@c$inputs] = %[[VAL_8]]#0 : <@Foo<[]>>, !pod.type<[@inp: !array.type<2 x !felt.type>]>
// CHECK-NEXT:        %[[VAL_23:[0-9a-zA-Z_\.]+]] = pod.read %[[VAL_4]][@comp] : <[@count: index, @comp: !struct.type<@Sum<[]>>, @params: !pod.type<[]>]>, !struct.type<@Sum<[]>>
// CHECK-NEXT:        struct.writef %[[VAL_2]][@c] = %[[VAL_23]] : <@Foo<[]>>, !struct.type<@Sum<[]>>
// CHECK-NEXT:        function.return %[[VAL_2]] : !struct.type<@Foo<[]>>
// CHECK-NEXT:      }
// CHECK-NEXT:      function.def @constrain(%[[VAL_24:[0-9a-zA-Z_\.]+]]: !struct.type<@Foo<[]>>, %[[VAL_25:[0-9a-zA-Z_\.]+]]: !array.type<2 x !felt.type>, %[[VAL_26:[0-9a-zA-Z_\.]+]]: !array.type<2 x !felt.type>) attributes {function.allow_constraint, function.allow_non_native_field_ops} {
// CHECK-NEXT:        %[[VAL_27:[0-9a-zA-Z_\.]+]] = struct.readf %[[VAL_24]][@c] : <@Foo<[]>>, !struct.type<@Sum<[]>>
// CHECK-NEXT:        %[[VAL_28:[0-9a-zA-Z_\.]+]] = struct.readf %[[VAL_24]][@c$inputs] : <@Foo<[]>>, !pod.type<[@inp: !array.type<2 x !felt.type>]>
...
// CHECK-NEXT:          %[[VAL_34:[0-9a-zA-Z_\.]+]] = bool.cmp le(%[[VAL_32]], %[[VAL_33]])
...
// CHECK-NEXT:        function.call @Sum::@constrain(%[[VAL_27]], %[[VAL_38]]) : (!struct.type<@Sum<[]>>, !array.type<2 x !felt.type>) -> ()
// CHECK-NEXT:        function.return
// CHECK-NEXT:      }
// CHECK-NEXT:    }
// CHECK-NEXT:    struct.def @Sum<[]> {
// CHECK-NEXT:      function.def @compute(%[[VAL_39:[0-9a-zA-Z_\.]+]]: !array.type<2 x !felt.type>) -> !struct.type<@Sum<[]>> attributes {function.allow_non_native_field_ops, function.allow_witness} {
// CHECK-NEXT:        %[[VAL_40:[0-9a-zA-Z_\.]+]] = struct.new : <@Sum<[]>>
// CHECK-NEXT:        %[[VAL_41:[0-9a-zA-Z_\.]+]] = felt.const  2
// CHECK-NEXT:        function.return %[[VAL_40]] : !struct.type<@Sum<[]>>
// CHECK-NEXT:      }
// CHECK-NEXT:      function.def @constrain(%[[VAL_42:[0-9a-zA-Z_\.]+]]: !struct.type<@Sum<[]>>, %[[VAL_43:[0-9a-zA-Z_\.]+]]: !array.type<2 x !felt.type>) attributes {function.allow_constraint, function.allow_non_native_field_ops} {
