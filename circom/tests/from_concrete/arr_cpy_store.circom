...
// CHECK-NEXT:        %[[VAL_4:[0-9a-zA-Z_\.]+]] = pod.new { @count = %[[VAL_3]] }  : <[@count: index, @comp: !struct.type<@Sum_0<[]>>, @params: !pod.type<[]>]>
...
// CHECK-NEXT:          %[[VAL_15:[0-9a-zA-Z_\.]+]] = pod.read %[[VAL_4]][@count] : <[@count: index, @comp: !struct.type<@Sum_0<[]>>, @params: !pod.type<[]>]>, index
// CHECK-NEXT:          %[[VAL_16:[0-9a-zA-Z_\.]+]] = arith.constant 1 : index
// CHECK-NEXT:          %[[VAL_17:[0-9a-zA-Z_\.]+]] = arith.subi %[[VAL_15]], %[[VAL_16]] : index
// CHECK-NEXT:          pod.write %[[VAL_4]][@count] = %[[VAL_17]] : <[@count: index, @comp: !struct.type<@Sum_0<[]>>, @params: !pod.type<[]>]>, index
...
// CHECK-NEXT:            %[[VAL_21:[0-9a-zA-Z_\.]+]] = function.call @Sum_0::@compute(%[[VAL_20]]) : (!array.type<2 x !felt.type>) -> !struct.type<@Sum_0<[]>>
// CHECK-NEXT:            pod.write %[[VAL_4]][@comp] = %[[VAL_21]] : <[@count: index, @comp: !struct.type<@Sum_0<[]>>, @params: !pod.type<[]>]>, !struct.type<@Sum_0<[]>>
// CHECK-NEXT:          } else {
// CHECK-NEXT:          }
...
// CHECK-NEXT:        function.call @Sum_0::@constrain(%[[VAL_27]], %[[VAL_38]]) : (!struct.type<@Sum_0<[]>>, !array.type<2 x !felt.type>) -> ()
// CHECK-NEXT:        function.return
// CHECK-NEXT:      }
// CHECK-NEXT:    }
// CHECK-NEXT:    struct.def @Sum_0<[]> {
// CHECK-NEXT:      function.def @compute(%[[VAL_39:[0-9a-zA-Z_\.]+]]: !array.type<2 x !felt.type>) -> !struct.type<@Sum_0<[]>> attributes {function.allow_non_native_field_ops, function.allow_witness} {
// CHECK-NEXT:        %[[VAL_40:[0-9a-zA-Z_\.]+]] = struct.new : <@Sum_0<[]>>
// CHECK-NEXT:        %[[VAL_41:[0-9a-zA-Z_\.]+]] = felt.const  2
// CHECK-NEXT:        function.return %[[VAL_40]] : !struct.type<@Sum_0<[]>>
// CHECK-NEXT:      }
// CHECK-NEXT:      function.def @constrain(%[[VAL_42:[0-9a-zA-Z_\.]+]]: !struct.type<@Sum_0<[]>>, %[[VAL_43:[0-9a-zA-Z_\.]+]]: !array.type<2 x !felt.type>) attributes {function.allow_constraint, function.allow_non_native_field_ops} {
