...
// CHECK-NEXT:      struct.field @c$inputs : !pod.type<[@inp: !array.type<@N x !felt.type>]>
...
// CHECK-NEXT:        %[[VAL_5:[0-9a-zA-Z_\.]+]] = pod.new { @count = %[[VAL_4]] }  : <[@count: index, @comp: !struct.type<@Sum<[@N]>>, @params: !pod.type<[]>]>
...
// CHECK-NEXT:          %[[VAL_10:[0-9a-zA-Z_\.]+]] = bool.cmp le(%[[VAL_9]], %[[VAL_9]])
...
// CHECK-NEXT:          %[[VAL_13:[0-9a-zA-Z_\.]+]] = pod.read %[[VAL_5]][@count] : <[@count: index, @comp: !struct.type<@Sum<[@N]>>, @params: !pod.type<[]>]>, index
// CHECK-NEXT:          %[[VAL_14:[0-9a-zA-Z_\.]+]] = arith.constant 1 : index
// CHECK-NEXT:          %[[VAL_15:[0-9a-zA-Z_\.]+]] = arith.subi %[[VAL_13]], %[[VAL_14]] : index
// CHECK-NEXT:          pod.write %[[VAL_5]][@count] = %[[VAL_15]] : <[@count: index, @comp: !struct.type<@Sum<[@N]>>, @params: !pod.type<[]>]>, index
...
// CHECK-NEXT:            %[[VAL_19:[0-9a-zA-Z_\.]+]] = function.call @Sum::@compute(%[[VAL_18]]) : (!array.type<@N x !felt.type>) -> !struct.type<@Sum<[@N]>>
// CHECK-NEXT:            pod.write %[[VAL_5]][@comp] = %[[VAL_19]] : <[@count: index, @comp: !struct.type<@Sum<[@N]>>, @params: !pod.type<[]>]>, !struct.type<@Sum<[@N]>>
// CHECK-NEXT:          } else {
// CHECK-NEXT:          }
...
// CHECK-NEXT:        }
// CHECK-NEXT:        struct.writef %[[VAL_2]][@c$inputs] = %[[VAL_7]]#0 : <@Foo<[@N]>>, !pod.type<[@inp: !array.type<@N x !felt.type>]>
// CHECK-NEXT:        %[[VAL_22:[0-9a-zA-Z_\.]+]] = pod.read %[[VAL_5]][@comp] : <[@count: index, @comp: !struct.type<@Sum<[@N]>>, @params: !pod.type<[]>]>, !struct.type<@Sum<[@N]>>
// CHECK-NEXT:        struct.writef %[[VAL_2]][@c] = %[[VAL_22]] : <@Foo<[@N]>>, !struct.type<@Sum<[@N]>>
// CHECK-NEXT:        function.return %[[VAL_2]] : !struct.type<@Foo<[@N]>>
// CHECK-NEXT:      }
// CHECK-NEXT:      function.def @constrain(%[[VAL_23:[0-9a-zA-Z_\.]+]]: !struct.type<@Foo<[@N]>>, %[[VAL_24:[0-9a-zA-Z_\.]+]]: !array.type<@N x !felt.type>, %[[VAL_25:[0-9a-zA-Z_\.]+]]: !array.type<@N x !felt.type>) attributes {function.allow_constraint, function.allow_non_native_field_ops} {
// CHECK-NEXT:        %[[VAL_26:[0-9a-zA-Z_\.]+]] = poly.read_const @N : !felt.type
// CHECK-NEXT:        %[[VAL_27:[0-9a-zA-Z_\.]+]] = struct.readf %[[VAL_23]][@c] : <@Foo<[@N]>>, !struct.type<@Sum<[@N]>>
// CHECK-NEXT:        %[[VAL_28:[0-9a-zA-Z_\.]+]] = struct.readf %[[VAL_23]][@c$inputs] : <@Foo<[@N]>>, !pod.type<[@inp: !array.type<@N x !felt.type>]>
// CHECK-NEXT:        %[[VAL_29:[0-9a-zA-Z_\.]+]] = scf.while (%[[VAL_30:[0-9a-zA-Z_\.]+]] = %[[VAL_26]]) : (!felt.type) -> !felt.type {
// CHECK-NEXT:          %[[VAL_31:[0-9a-zA-Z_\.]+]] = bool.cmp le(%[[VAL_30]], %[[VAL_30]])
...
// CHECK-NEXT:        function.call @Sum::@constrain(%[[VAL_27]], %[[VAL_36]]) : (!struct.type<@Sum<[@N]>>, !array.type<@N x !felt.type>) -> ()
// CHECK-NEXT:        function.return
// CHECK-NEXT:      }
// CHECK-NEXT:    }
// CHECK-NEXT:    struct.def @Sum<[@N]> {
// CHECK-NEXT:      function.def @compute(%[[VAL_37:[0-9a-zA-Z_\.]+]]: !array.type<@N x !felt.type>) -> !struct.type<@Sum<[@N]>> attributes {function.allow_non_native_field_ops, function.allow_witness} {
// CHECK-NEXT:        %[[VAL_38:[0-9a-zA-Z_\.]+]] = struct.new : <@Sum<[@N]>>
// CHECK-NEXT:        %[[VAL_39:[0-9a-zA-Z_\.]+]] = poly.read_const @N : !felt.type
// CHECK-NEXT:        function.return %[[VAL_38]] : !struct.type<@Sum<[@N]>>
// CHECK-NEXT:      }
// CHECK-NEXT:      function.def @constrain(%[[VAL_40:[0-9a-zA-Z_\.]+]]: !struct.type<@Sum<[@N]>>, %[[VAL_41:[0-9a-zA-Z_\.]+]]: !array.type<@N x !felt.type>) attributes {function.allow_constraint, function.allow_non_native_field_ops} {
