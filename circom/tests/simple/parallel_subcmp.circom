...
// CHECK-NEXT:        struct.writef %[[VAL_2]][@foo$inputs] = %[[VAL_6]] : <@Parallel<[]>>, !pod.type<[]>
// CHECK-NEXT:        %[[VAL_7:[0-9a-zA-Z_\.]+]] = pod.read %[[VAL_5]][@comp] : <[@count: index, @comp: !struct.type<@Foo<[]>>, @params: !pod.type<[]>]>, !struct.type<@Foo<[]>>
// CHECK-NEXT:        struct.writef %[[VAL_2]][@foo] = %[[VAL_7]] : <@Parallel<[]>>, !struct.type<@Foo<[]>>
...
// CHECK-NEXT:        %[[VAL_9:[0-9a-zA-Z_\.]+]] = struct.readf %[[VAL_8]][@foo] : <@Parallel<[]>>, !struct.type<@Foo<[]>>
// CHECK-NEXT:        %[[VAL_10:[0-9a-zA-Z_\.]+]] = struct.readf %[[VAL_8]][@foo$inputs] : <@Parallel<[]>>, !pod.type<[]>
