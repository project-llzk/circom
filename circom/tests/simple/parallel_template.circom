...
// CHECK-NEXT:      }
// CHECK-NEXT:      function.def @constrain(%[[VAL_1:[0-9a-zA-Z_\.]+]]: !struct.type<@Foo<[]>>) attributes {function.allow_constraint} {
// CHECK-NEXT:        function.return
// CHECK-NEXT:      }
// CHECK-NEXT:    }
// CHECK-NEXT:    struct.def @Parallel<[]> {
// CHECK-NEXT:      struct.field @foo : !struct.type<@Foo<[]>>
// CHECK-NEXT:      function.def @compute() -> !struct.type<@Parallel<[]>> attributes {function.allow_witness} {
...
// CHECK-NEXT:      }
// CHECK-NEXT:      function.def @constrain(%[[VAL_4:[0-9a-zA-Z_\.]+]]: !struct.type<@Parallel<[]>>) attributes {function.allow_constraint} {
