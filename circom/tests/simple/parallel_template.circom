...
// CHECK-LABEL: module attributes {llzk.lang, llzk.main = !struct.type<@Parallel<[]>>} {
...
// CHECK-NEXT:          %[[VAL_4:[0-9a-zA-Z_\.]+]] = function.call @Foo::@Foo::@compute() : () -> !struct.type<@Foo::@Foo<[]>>
// CHECK-NEXT:          %[[VAL_5:[0-9a-zA-Z_\.]+]] = pod.new { @comp = %[[VAL_4]] }  : <[@count: index, @comp: !struct.type<@Foo::@Foo<[]>>, @params: !pod.type<[]>]>
// CHECK-NEXT:          %[[VAL_6:[0-9a-zA-Z_\.]+]] = pod.new : <[]>
// CHECK-NEXT:          struct.writem %[[VAL_2]][@foo$inputs] = %[[VAL_6]] : <@Parallel::@Parallel<[]>>, !pod.type<[]>
// CHECK-NEXT:          %[[VAL_7:[0-9a-zA-Z_\.]+]] = pod.read %[[VAL_5]][@comp] : <[@count: index, @comp: !struct.type<@Foo::@Foo<[]>>, @params: !pod.type<[]>]>, !struct.type<@Foo::@Foo<[]>>
// CHECK-NEXT:          struct.writem %[[VAL_2]][@foo] = %[[VAL_7]] : <@Parallel::@Parallel<[]>>, !struct.type<@Foo::@Foo<[]>>
// CHECK-NEXT:          function.return %[[VAL_2]] : !struct.type<@Parallel::@Parallel<[]>>
// CHECK-NEXT:        }
// CHECK-NEXT:        function.def @constrain(%[[VAL_8:[0-9a-zA-Z_\.]+]]: !struct.type<@Parallel::@Parallel<[]>>) attributes {function.allow_constraint, function.allow_non_native_field_ops} {
// CHECK-NEXT:          %[[VAL_9:[0-9a-zA-Z_\.]+]] = struct.readm %[[VAL_8]][@foo] : <@Parallel::@Parallel<[]>>, !struct.type<@Foo::@Foo<[]>>
// CHECK-NEXT:          %[[VAL_10:[0-9a-zA-Z_\.]+]] = struct.readm %[[VAL_8]][@foo$inputs] : <@Parallel::@Parallel<[]>>, !pod.type<[]>
// CHECK-NEXT:          function.call @Foo::@Foo::@constrain(%[[VAL_9]]) : (!struct.type<@Foo::@Foo<[]>>) -> ()
// CHECK-NEXT:          function.return
// CHECK-NEXT:        }
// CHECK-NEXT:      }
// CHECK-NEXT:    }
